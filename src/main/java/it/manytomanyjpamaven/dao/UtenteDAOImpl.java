...
public class UtenteDAOImpl implements UtenteDAO {
	...
	public List<Utente> findAllCreateAtJune() {
		TypedQuery<Utente> query = entityManager.createQuery(
				"select u FROM Utente u where u.dateCreated < '2021-07-01' and u.dateCreated > '2021-06-01' ",
				Utente.class);
		return query.getResultList();
	}
	...
	public Long countAdmin() {
		TypedQuery<Long> query = entityManager
				.createQuery("select count (u) FROM Utente u join u.ruoli r where r.id =1 ", Long.class);
		return query.getSingleResult();
	}

	public List<Utente> findAllByPasswordConMenoDiOttoCaratteri() {
		TypedQuery<Utente> query = entityManager
				.createQuery("select u FROM Utente u where u.password not like '_______%' ", Utente.class);
		return query.getResultList();
	}

	public Long countAdminDisabilitati() {
		TypedQuery<Long> query = entityManager.createQuery(
				"select count (u) FROM Utente u join u.ruoli r where r.id=1 and u.stato='DISABILITATO' ", Long.class);
		return query.getSingleResult();
	}
